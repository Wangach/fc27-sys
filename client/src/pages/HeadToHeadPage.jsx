...
export default function HeadToHeadPage() {
  ...
  const compare = async () => {
    if (!playerOneId || !playerTwoId)
      return setError("Select two players to compare.");
    if (playerOneId === playerTwoId)
      return setError("Choose two different players.");
    setLoading(true);
    setError("");
    setComparison(null);
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Could not calculate head-to-head statistics.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <button
          ...
          onClick={compare}
          disabled={loading || !playerOneId || !playerTwoId}
        >
...
function PlayerPicker({ label, players, value, onChange, excludeId }) {
  ...
  const filtered = useMemo(() => {
    ...
    return players
      ...
      .filter(
        (player) =>
          !term ||
          `${player.displayName} ${player.customerCode} ${player.favoriteTeam || ""}`
            .toLowerCase()
            .includes(term),
      )
      .slice(0, 8);
  ...
  return (
    <section className="panel p-5">
      ...
      />
      {selected && (
        <div className="mt-3 rounded-xl border border-volt/30 bg-volt/[.07] p-3">
          <div className="text-xs font-black uppercase tracking-wider text-volt">
            Selected
          </div>
          <div className="mt-1 font-black">{selected.displayName}</div>
          <div className="text-xs text-white/35">
            {selected.customerCode} •{" "}
            {selected.favoriteTeam || "No favourite team"}
          </div>
        </div>
      )}
      <div className="mt-3 max-h-52 space-y-1 overflow-y-auto">
        {filtered.map((player) => (
          ...
          >
            ...
            <div
              className={`text-xs ${value === player.id ? "text-arena-950/60" : "text-white/35"}`}
            >
              {player.customerCode} •{" "}
              {player.favoriteTeam || "No favourite team"}
            </div>
          </button>
        ))}
        {!filtered.length && (
          <div className="rounded-xl bg-white/[.025] p-3 text-sm text-white/35">
            No matching players.
          </div>
        )}
      </div>
  ...
  return (
    <div className="mt-7 space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          icon={faGamepad}
          label="Direct meetings"
          value={headToHead.meetings}
        />
        <StatCard icon={faTrophy} label="Draws" value={headToHead.draws} />
        <StatCard
          icon={faFutbol}
          label={`${playerOne.displayName} goals`}
          value={headToHead.playerOneGoals}
        />
        <StatCard
          icon={faFutbol}
          label={`${playerTwo.displayName} goals`}
          value={headToHead.playerTwoGoals}
        />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_.72fr_1fr]">
        <PlayerComparisonCard
          ...
          directRate={
            headToHead.meetings ? headToHead.playerOneHeadToHeadWinRate : null
          }
        ...
        <section className="panel flex flex-col justify-center p-5 text-center">
          ...
          <p className="mt-2 text-[11px] text-white/25">
            This is a statistical estimate from recorded matches, not a
            guaranteed result.
          </p>
        </section>
        ...
        <PlayerComparisonCard
          ...
          directRate={
            headToHead.meetings ? headToHead.playerTwoHeadToHeadWinRate : null
          }
      ...
      <section className="panel p-5">
        <div className="mb-4">
          ...
          <p className="text-xs text-white/35">
            Latest 5 games between these two players
          </p>
        </div>
        {recentMeetings.length ? (
            ...
            <table className="data-table">
              <thead>
                <tr>
                  <th>Date</th>
                  <th>Match</th>
                  <th>{playerOne.displayName}</th>
                  <th>Score</th>
                  <th>{playerTwo.displayName}</th>
                  <th>Type</th>
                </tr>
              </thead>
              <tbody>
                {recentMeetings.map((match) => (
                  <tr key={match.id}>
                    ...
                    <td className="font-black text-volt">
          ...
          <div className="rounded-xl bg-white/[.03] p-5 text-sm text-white/40">
            These players have not faced each other yet. The estimate above
            therefore uses their overall records only.
          </div>
        )}
      </section>
    </div>
...
function PlayerComparisonCard({ player, wins, goals, directRate, estimate }) {
  ...
  return (
    <section className="panel panel-cut p-5">
      <div className="eyebrow">{player.customerCode}</div>
      <h2 className="mt-1 text-2xl font-black uppercase">
        {player.displayName}
      </h2>
      <div className="mt-1 text-xs text-white/35">
        Favourite team: {player.favoriteTeam || "Not set"} • Rank{" "}
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        ...
        <Metric
          label="H2H win rate"
          value={directRate == null ? "—" : `${directRate}%`}
        />
        <Metric label="Overall win rate" value={`${overall.winRate}%`} />
        <Metric
          label="Overall record"
          value={`${overall.wins}W ${overall.draws}D ${overall.losses}L`}
        />
        <Metric
          label="Matchup estimate"
...
function Metric({ label, value, emphasis = false }) {
  return (
    <div className="rounded-xl bg-white/[.035] p-3">
      <div className="text-[10px] font-black uppercase tracking-[.12em] text-white/35">
        {label}
      </div>
      <div
        className={`mt-1 font-black ${emphasis ? "text-xl text-volt" : "text-lg"}`}
      >
        {value}
      </div>
    </div>
...
function Probability({ label, value }) {
  return (
    <div className="rounded-xl bg-white/[.04] p-2">
      <div className="truncate text-[10px] uppercase text-white/35">
        {label}
      </div>
