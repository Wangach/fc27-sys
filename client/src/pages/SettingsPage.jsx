  ...
  const save = async (type, index) => {
    ...
    try {
      ...
      const response = await api.patch(
        `/settings/match-types/${type.code}`,
        payload,
      );
  ...
  return (
    <>
            ...
            <div key={type.id} className="panel p-5">
              ...
              <div className="mt-5">
                ...
                <input
                  ...
                  onChange={(event) =>
                    updateType(index, {
                      [isFairPay ? "perPlayerFee" : "loserFee"]:
                        event.target.value,
                    })
                  }
                />
              ...
              <label className="mt-4 flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(event) =>
                    updateType(index, { active: event.target.checked })
                  }
                />
                Active match type
              </label>
      ...
      <div className="panel mt-6 p-5 text-sm text-white/50">
        <strong className="text-white">Billing rule:</strong> Loser Pay uses the
        loser fee. Fair Pay uses the fee-per-player value and creates one game
        charge for each participant. A Fair Pay fee of KES 0 means no financial
        charge will be created for Fair Pay matches.
      </div>
    </>
  );
}
