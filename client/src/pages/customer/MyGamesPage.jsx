...
export default function MyGamesPage() {
  ...
  return (
    <>
      <PageHeader
        title="My Games"
        subtitle="Complete match history, results, teams used and game type."
      />
      <div className="space-y-3">
        {rows.map((row) => {
          const match = row.match;
          const [a, b] = match.participants;
          const myCharge = match.charges?.find(
            (charge) => charge.customerId === row.customerId,
          );
                ...
                <div>
                  ...
                  {match.drawResolution === "SUPER_LOSER" && (
                    <div className="mt-2 text-xs font-bold text-orange-200">
                      Super-looser tiebreak: {match.superLoserPlayerOneScore}:
                      {match.superLoserPlayerTwoScore}
                      {myCharge
                        ? ` • You were charged ${kes(myCharge.amount)}`
                        : " • No game charge to you"}
                    </div>
                  )}
                </div>
