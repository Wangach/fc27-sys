...
import { faScaleBalanced, faSkullCrossbones, faXmark } from '@fortawesome/free-solid-svg-icons';
...
export default function GamesPage() {
  ...
  const [superScores, setSuperScores] = useState({ playerOne: 0, playerTwo: 0 });
  ...
  const loserPayType = useMemo(() => types.find((type) => type.code === 'LOSER_PAY'), [types]);
  ...
  const submit = async (event) => {
    ...
    if (form.matchTypeCode === 'LOSER_PAY' && Number(form.playerOneScore) === Number(form.playerTwoScore)) {
      setDrawMode(null);
      setDrawError('');
      setSuperScores({ playerOne: 0, playerTwo: 0 });
  ...
  const submitSuperLoser = async () => {
    ...
    if (playerOneScore === playerTwoScore) {
      setDrawError('The Super-looser score must produce a winner and a loser. It cannot also be a draw.');
      return;
    }
    await recordMatch({
      ...form,
    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[.8fr_1.2fr]">
        <form onSubmit={submit} className="panel p-5">
          ...
          {error && <div className="mb-4 rounded-xl bg-red-400/10 p-3 text-sm text-red-200">{error}</div>}

          <label className="label">Match type</label>
          <select
            className="field mb-4"
            value={form.matchTypeCode}
            onChange={(e) => setForm({ ...form, matchTypeCode: e.target.value })}
          >
            {types.map((type) => (
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <select
                ...
                onChange={(e) => setForm({ ...form, playerOneId: e.target.value })}
              >
              ...
              <input
                ...
                placeholder={teams.length ? 'Search/select team' : 'Import teams first'}
                value={form.playerOneTeam}
                onChange={(e) => setForm({ ...form, playerOneTeam: e.target.value })}
              />
              <input
                ...
                onChange={(e) => setForm({ ...form, playerOneScore: Number(e.target.value) })}
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setForm({ ...form, playerTwoId: e.target.value })}
              >
                <option value="">Select</option>
                {customers.map((customer) => (
                  <option key={customer.id} value={customer.id}>
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, playerTwoTeam: e.target.value })}
              />
              <input
                className="field mt-2 text-center text-2xl font-black"
                type="number"
                ...
                onChange={(e) => setForm({ ...form, playerTwoScore: Number(e.target.value) })}
              />
            </div>
          </div>
        ...
        <div className="panel p-5">
          ...
          <div className="max-h-[650px] space-y-3 overflow-auto pr-1">
            {matches.map((match) => {
              ...
              return (
                <div key={match.id} className="rounded-xl border border-white/[.07] p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-white/40">
                        {a?.team || '—'} vs {b?.team || '—'} • {match.matchType.name}
                      </div>

                      {match.drawResolution === 'HALF_HALF' && (
                        <div className="mt-2 text-xs font-bold text-amber-200">
                          Draw resolved as Half-Half • {match.charges.map((charge) => `${charge.customer.displayName} ${kes(charge.amount)}`).join(' • ')}
                        </div>
                      )}

                      {match.drawResolution === 'SUPER_LOSER' && (
                        <div className="mt-2 text-xs font-bold text-orange-200">
                          Super-looser {match.superLoserPlayerOneScore}:{match.superLoserPlayerTwoScore} •{' '}
                          {match.charges.map((charge) => `${charge.customer.displayName} charged ${kes(charge.amount)}`).join(' • ')}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!match.drawResolution && match.charges?.length > 0 && (
                        <span className="text-xs font-bold text-volt">
                          {match.charges.map((charge) => `${charge.customer.displayName} ${kes(charge.amount)}`).join(' • ')}
                        </span>
                      )}
                      {user.role === 'ADMIN' && match.status === 'ACTIVE' && (
                        <button
                          onClick={async () => {
                            if (confirm('Cancel this match and void its active game charge?')) {
          ...
          <div className="w-full max-w-xl rounded-3xl border border-white/10 bg-[#111411] p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-1 text-2xl font-black uppercase">Who pays for this match?</h2>
                <p className="mt-2 text-sm text-white/50">
                  {playerOne?.displayName || 'Player 1'} and {playerTwo?.displayName || 'Player 2'} finished{' '}
                  <strong className="text-white">{form.playerOneScore}:{form.playerTwoScore}</strong>. Choose how the KES {normalLoserFee || 30} game cost should be resolved.
                </p>
              </div>
              <button
                type="button"
            ...
            {drawError && <div className="mt-4 rounded-xl bg-red-400/10 p-3 text-sm text-red-200">{drawError}</div>}
            ...
            {!drawMode && (
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                ...
                >
                  <FontAwesomeIcon icon={faScaleBalanced} className="text-xl text-volt" />
                  <div className="mt-4 text-lg font-black uppercase">Half-Half</div>
                  <div className="mt-1 text-sm text-white/45">
                    Split one normal game cost equally: <strong className="text-white">KES {(normalLoserFee / 2 || 15).toFixed(2)} each</strong>.
                  </div>
                </button>
                ...
                >
                  <FontAwesomeIcon icon={faSkullCrossbones} className="text-xl text-orange-300" />
                  <div className="mt-4 text-lg font-black uppercase">Super-looser</div>
                  <div className="mt-1 text-sm text-white/45">
                    Enter a decisive tiebreak score. The tiebreak loser pays <strong className="text-white">KES {(normalLoserFee * 2 || 60).toFixed(2)}</strong>.
                  </div>
                </button>
              </div>
            )}
            ...
            {drawMode === 'SUPER_LOSER' && (
              <div className="mt-6">
                <div className="rounded-2xl border border-orange-300/15 bg-orange-300/5 p-4">
                  <div className="text-sm font-bold text-orange-200">Enter the Super-looser tiebreak score</div>
                  <div className="mt-1 text-xs text-white/45">
                    This score determines only who pays. The original match remains recorded as a {form.playerOneScore}:{form.playerTwoScore} draw for player statistics.
                  </div>
                </div>

                <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-end gap-3">
                  <div>
                    <label className="label">{playerOne?.displayName || 'Player 1'}</label>
                    <input
                      ...
                      onChange={(e) => setSuperScores({ ...superScores, playerOne: Number(e.target.value) })}
                    />
                  </div>
                  <div className="pb-3 text-xl font-black text-white/30">:</div>
                  <div>
                    <label className="label">{playerTwo?.displayName || 'Player 2'}</label>
                    <input
                      className="field text-center text-2xl font-black"
                      type="number"
                      min="0"
                      ...
                      onChange={(e) => setSuperScores({ ...superScores, playerTwo: Number(e.target.value) })}
                    />
                  </div>
                </div>
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                  ...
                  >
                    {saving ? 'Recording…' : `Record & charge KES ${(normalLoserFee * 2 || 60).toFixed(2)}`}
                  </button>
                </div>
              </div>
            )}

            {!drawMode && saving && <div className="mt-4 text-center text-sm text-white/40">Recording match…</div>}
          </div>
        </div>
      )}
    </>
