...
function mapRecentGame(row, customerId) {
  ...
  const self =
    match.participants.find((p) => p.customerId === customerId) || row;
...
export async function getPlayerOverview(db, customerId) {
  ...
  return {
    ...
    recentGames: recentParticipants.map((row) =>
      mapRecentGame(row, customerId),
    ),
  };
}
...
function buildPrediction(playerOne, playerTwo, direct) {
  ...
  const p1Overall = playerOne.played
    ? safeRate(playerOne.wins, playerOne.played)
    : 0.5;
  const p2Overall = playerTwo.played
    ? safeRate(playerTwo.wins, playerTwo.played)
    : 0.5;
  const overallDraw =
    playerOne.played || playerTwo.played
      ? (safeRate(playerOne.draws, playerOne.played || 1) +
          safeRate(playerTwo.draws, playerTwo.played || 1)) /
        2
      : 0;

  const p1Direct = direct.meetings
    ? safeRate(direct.playerOneWins, direct.meetings)
    : 0;
  const p2Direct = direct.meetings
    ? safeRate(direct.playerTwoWins, direct.meetings)
    : 0;
  const directDraw = direct.meetings
    ? safeRate(direct.draws, direct.meetings)
    : 0;
  ...
  const drawProbability = round1(
    Math.max(0, 100 - playerOneWinProbability - playerTwoWinProbability),
  );
  ...
  const likelyOutcome =
    outcomes[0].value - outcomes[1].value < 5
      ? "Too close to call"
      : outcomes[0].label;
...
export async function getHeadToHeadComparison(db, playerOneId, playerTwoId) {
  ...
  const overallOne = leaderboard.find(
    (row) => row.customerId === playerOneId,
  ) || { played: 0, wins: 0, draws: 0, losses: 0, winRate: 0, rank: null };
  const overallTwo = leaderboard.find(
    (row) => row.customerId === playerTwoId,
  ) || { played: 0, wins: 0, draws: 0, losses: 0, winRate: 0, rank: null };
  ...
  const direct = {
    ...
    playerOneGoals,
    playerTwoGoals,
    playerOneHeadToHeadWinRate: meetings.length
      ? round1((playerOneWins / meetings.length) * 100)
      : 0,
    playerTwoHeadToHeadWinRate: meetings.length
      ? round1((playerTwoWins / meetings.length) * 100)
      : 0,
