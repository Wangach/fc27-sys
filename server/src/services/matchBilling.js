...
export function getLoserPayCharges({
  ...
  if (!drawResolution) {
    const error = new Error('A loser-pay draw requires a Half-Half or Super-looser decision.');
  ...
  if (drawResolution === 'SUPER_LOSER') {
    if (!Number.isInteger(superLoserPlayerOneScore) || !Number.isInteger(superLoserPlayerTwoScore)) {
      const error = new Error('Super-looser requires a new score for both players.');
      error.code = 'SUPER_LOSER_SCORES_REQUIRED';
      throw error;
    }
    if (superLoserPlayerOneScore === superLoserPlayerTwoScore) {
      const error = new Error('The Super-looser score cannot also be a draw. Enter a decisive score.');
      error.code = 'SUPER_LOSER_DRAW_NOT_ALLOWED';
      throw error;
    }
    const loserId = superLoserPlayerOneScore < superLoserPlayerTwoScore ? playerOneId : playerTwoId;
