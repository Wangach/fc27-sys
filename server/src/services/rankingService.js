export async function getLeaderboard(db) {
  ...
  const table = customers.map((c) => {
    ...
    return { customerId: c.id, customerCode: c.customerCode, name: c.displayName, favoriteTeam: c.favoriteTeam, played: wins + draws + losses, wins, draws, losses, gf, ga, gd: gf - ga, points: wins * 3 + draws, winRate: wins + draws + losses ? Number(((wins / (wins + draws + losses)) * 100).toFixed(1)) : 0 };
  });

  // Goals against requires both participants, so fetch matches once and fold scores safely.
  const matches = await db.match.findMany({ where: { status: 'ACTIVE' }, include: { participants: true } });
  ...
  for (const match of matches) {
    ...
    const ra = byId.get(a.customerId); const rb = byId.get(b.customerId);
    if (ra) ra.ga += b.score;
    if (rb) rb.ga += a.score;
  }
  for (const row of table) row.gd = row.gf - row.ga;

  table.sort((a, b) => b.points - a.points || b.gd - a.gd || b.gf - a.gf || b.winRate - a.winRate || a.name.localeCompare(b.name));
