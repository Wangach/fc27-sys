  ...
  const ticket = await prisma.$transaction(async (tx) => {
    const created = await tx.ticket.create({
      data: {
        ...
        messages: { create: { senderId: req.user.id, message: data.message } },
      },
      include: { messages: true },
    });
    await writeAudit(tx, req, "TICKET_CREATED", "Ticket", created.id, {
      type: data.type,
      subject: data.subject,
    });
    return created;
...
router.post("/:id/messages", async (req, res) => {
  const { message } = z
    .object({ message: z.string().min(1).max(3000) })
    .parse(req.body);
  ...
  if (
    ...
    ticket.customerId !== req.user.customerProfile?.id
  )
    return res.status(403).json({ message: "You cannot access this ticket." });
  const entry = await prisma.ticketMessage.create({
    data: { ticketId: ticket.id, senderId: req.user.id, message },
  });
