...
router.get("/leaderboard", async (req, res) =>
  res.json({ leaderboard: await getLeaderboard(prisma) }),
);
...
router.get("/head-to-head", authorize("ADMIN"), async (req, res) => {
  ...
  const comparison = await getHeadToHeadComparison(
    prisma,
    String(playerOneId),
    String(playerTwoId),
  );
  if (!comparison)
    return res
      .status(404)
      .json({ message: "One or both players could not be found." });
  res.json(comparison);
});
...
router.get(
  ...
  async (req, res) => {
    ...
    for (const c of debts)
      totalDebt += (await getCustomerAccount(prisma, c.id)).totalDebt;
