...
router.get(
  ...
  async (req, res) => {
    res.json({
      account: await getCustomerAccount(prisma, req.params.customerId),
    });
  },
);
...
router.get("/my-summary", authorize("CUSTOMER"), async (req, res) => {
  res.json({
    account: await getCustomerAccount(prisma, req.user.customerProfile.id),
  });
});
...
router.get(
  ...
  async (req, res) => {
    const where = req.query.customerId
      ...
      : {};
    const debts = await prisma.purchaseDebt.findMany({
      where,
      include: {
        customer: true,
      ...
      debts: debts.map((d) => ({
        ...
        outstanding: roundMoney(
          Math.max(
            0,
            money(d.amount) -
              d.allocations.reduce((s, a) => s + money(a.amount), 0),
          ),
        ),
      })),
    });
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!customer || !customer.active)
      return res.status(400).json({ message: "Active customer not found." });
    const debt = await prisma.$transaction(async (tx) => {
      const created = await tx.purchaseDebt.create({
        data: {
          debtCode: makeCode("DEBT"),
      ...
      await writeAudit(
        tx,
        req,
        "PURCHASE_DEBT_CREATED",
        "PurchaseDebt",
        created.id,
        { debtCode: created.debtCode, amount: data.amount },
      );
      return created;
    });
...
router.post(
  "/purchase-debts/:id/cancel",
  authorize("ADMIN"),
  async (req, res) => {
    const debt = await prisma.purchaseDebt.findUnique({
      where: { id: req.params.id },
      include: { allocations: { where: { payment: { status: "ACTIVE" } } } },
    });
    if (!debt) return res.status(404).json({ message: "Debt not found." });
    if (debt.allocations.length)
      return res
        .status(409)
        .json({
          message:
            "A debt with active payments cannot be cancelled. Reverse/reallocate the payment first.",
        });
...
router.post("/payments", authorize("ADMIN", "CO_ADMIN"), async (req, res) => {
  ...
  const payment = await prisma.$transaction(async (tx) => {
    ...
    if (data.purpose === "PURCHASE_DEBT") {
      ...
      if (data.targetPurchaseDebtId && debts.length === 0)
        throw Object.assign(
          new Error(
            "Selected purchase debt is not available for this customer.",
          ),
      ...
      for (const debt of debts) {
        ...
        await tx.purchaseDebt.update({
          where: { id: debt.id },
          data: {
            status: after <= 0 ? "PAID" : "PARTIALLY_PAID",
            paidAt: after <= 0 ? new Date() : null,
          },
        });
      }
    }
    ...
    await writeAudit(tx, req, "PAYMENT_RECORDED", "Payment", created.id, {
...
router.post("/payments/:id/reverse", authorize("ADMIN"), async (req, res) => {
  ...
  if (payment.status === "REVERSED")
    return res.json({ message: "Payment is already reversed." });

  await prisma.$transaction(async (tx) => {
    await tx.payment.update({
      where: { id: payment.id },
      data: { status: "REVERSED", reversedAt: new Date(), unappliedAmount: 0 },
    });
    const debtIds = [
      ...new Set(
        payment.allocations.map((a) => a.purchaseDebtId).filter(Boolean),
      ),
    ];
    for (const debtId of debtIds) {
      const debt = await tx.purchaseDebt.findUnique({
        where: { id: debtId },
        include: {
          allocations: {
            where: { payment: { status: "ACTIVE", id: { not: payment.id } } },
          },
        },
      });
      ...
      await tx.purchaseDebt.update({
        ...
        data: {
          status:
            outstanding <= 0 ? "PAID" : paid > 0 ? "PARTIALLY_PAID" : "UNPAID",
          paidAt: outstanding <= 0 ? debt.paidAt || new Date() : null,
        },
      });
    }
    await writeAudit(tx, req, "PAYMENT_REVERSED", "Payment", payment.id, {
      paymentNumber: payment.paymentNumber,
    });
  });
  res.json({
    message: "Payment reversed. Debt balances have been recalculated.",
  });
});
