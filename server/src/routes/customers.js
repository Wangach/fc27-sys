...
router.get('/', authorize('ADMIN', 'CO_ADMIN'), async (req, res) => {
  const customers = await prisma.customerProfile.findMany({
    ...
    select: { id: true, customerCode: true, displayName: true, email: true, phone: true, favoriteTeam: true, createdAt: true },
...
router.get('/me', authorize('CUSTOMER'), async (req, res) => {
  ...
  const [account, leaderboard, recentMatches, recentPayments] = await Promise.all([
    getCustomerAccount(prisma, profile.id),
    getLeaderboard(prisma),
    prisma.matchParticipant.findMany({ where: { customerId: profile.id, match: { status: 'ACTIVE' } }, include: { match: { include: { matchType: true, participants: { include: { customer: true } } } } }, orderBy: { match: { playedAt: 'desc' } }, take: 10 }),
    prisma.payment.findMany({ where: { customerId: profile.id, status: 'ACTIVE' }, orderBy: { createdAt: 'desc' }, take: 10 }),
  ]);
  const row = leaderboard.find((x) => x.customerId === profile.id) || null;
  res.json({ profile, account, stats: row, recentMatches, recentPayments });
});

router.get('/:id/insights', authorize('ADMIN', 'CO_ADMIN'), async (req, res) => {
  const overview = await getPlayerOverview(prisma, req.params.id);
  if (!overview) return res.status(404).json({ message: 'Customer not found.' });
  res.json(overview);
});

router.get('/:id', authorize('ADMIN', 'CO_ADMIN'), async (req, res) => {
  const customer = await prisma.customerProfile.findUnique({ where: { id: req.params.id }, include: { user: { select: { username: true, status: true } } } });
