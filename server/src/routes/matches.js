...
router.post("/", authorize("ADMIN", "CO_ADMIN"), async (req, res) => {
  ...
  if (data.playerOneId === data.playerTwoId) {
    return res
      .status(400)
      .json({ message: "A player cannot play against themselves." });
  }

  const matchType = await prisma.matchType.findUnique({
    where: { code: data.matchTypeCode },
  });
  if (!matchType || !matchType.active) {
    return res
      .status(400)
      .json({ message: "Selected match type is unavailable." });
  }
  ...
  if (players.length !== 2) {
    return res
      .status(400)
      .json({ message: "Both players must be active customers." });
  }
  ...
  if (matchType.code !== "LOSER_PAY" && data.drawResolution) {
    return res
      .status(400)
      .json({
        message: "Draw payment resolution is only used for loser-pay matches.",
      });
  }
  if (matchType.code === "LOSER_PAY" && !isDraw && data.drawResolution) {
    return res
      ...
      .json({
        message:
          "Draw payment resolution cannot be used when the match has a winner.",
      });
  }
  ...
  const match = await prisma.$transaction(async (tx) => {
    const created = await tx.match.create({
      data: {
        ...
        drawResolution:
          matchType.code === "LOSER_PAY" && isDraw ? data.drawResolution : null,
        superLoserPlayerOneScore:
          matchType.code === "LOSER_PAY" &&
          isDraw &&
          data.drawResolution === "SUPER_LOSER"
...
router.post("/:id/cancel", authorize("ADMIN"), async (req, res) => {
  ...
  if (match.status === "CANCELLED")
    return res.json({ message: "Match is already cancelled." });
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.gameCharge.updateMany({
      where: { matchId: match.id, voidedAt: null },
      data: { voidedAt: new Date() },
    });
    await writeAudit(tx, req, "MATCH_CANCELLED", "Match", match.id, {
      matchCode: match.matchCode,
    });
  });

  res.json({
    message: "Match cancelled and its active game charges were voided.",
  });
});
