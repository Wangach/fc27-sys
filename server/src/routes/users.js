...
router.post("/", authorize("ADMIN"), async (req, res) => {
  ...
  if (data.role === "CUSTOMER" && !data.displayName)
    return res
      .status(400)
      .json({ message: "Customer display name is required." });
  ...
  const user = await prisma.$transaction(async (tx) => {
    ...
    await writeAudit(tx, req, "USER_CREATED", "User", created.id, {
      role: data.role,
...
router.patch("/:id", authorize("ADMIN"), async (req, res) => {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    await writeAudit(tx, req, "USER_UPDATED", "User", existing.id, {
